...
public class SinglyLinkedList<E> implements List<E>{
	...
	public SinglyLinkedList(Comparable<E>[] keys){
		this.add(0, ((E)keys[0]));
		for(int i = 1; i < keys.length; i++){
			boolean inserted = false;
			for(int j = 0; j < this.size() && !inserted; j++){
				if(keys[i].compareTo(this.get(j)) >= 0){
					this.add(j + 1, ((E) keys[i]));
					inserted = true;
				}
			}
			if(!inserted) this.add(this.size(), ((E)keys[i]));
		}
	}
